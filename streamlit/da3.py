...
#    monthly_performance = performance_df.groupby(['Month', 'Practitioner ID']).agg(
 #       Total_Hours=('Total Hours', 'sum'),
#        Total_Income=('Total Income', 'sum')
#    ).reset_index()
...
def display_performance_analytics(weekly_performance):#, monthly_data):
    ...
    # Monthly performance chart (bar chart comparing actual vs. target hours)
   ...
   #     x='Month',
    ...
    #    color='Practitioner ID',
   #     barmode='group',
   #     title="Monthly Hours Worked vs Target",
   #     labels={'value': 'Hours', 'variable': 'Legend'}
  #  )
...
def master_dashboard(performance_df, target_df):
    ...
    #st.write("Columns in weekly_performance:", weekly_performance.columns)
...
# Function to update target income and hours for a practitioner
#def update_target(practitioner_id, week, appt_type, new_target_income=None, new_target_hours=None):
 #   try:
        # Load the Excel file
        #target_file_path = r"C:\Users\divfi\Desktop\Sem4 DA\Capstone\Re_ Dashboard project Langara\clinic_target.xlsx"
        #workbook = openpyxl.load_workbook(target_file_path)
        #sheet = workbook.active  # Assumes data is in the active sheet

        # Find the row with the matching practitioner_id, week, and appointment type
        #row_to_update = None
        #for row in sheet.iter_rows(min_row=2, values_only=False):  # Start from row 2 to skip header
          #  if row[0].value == practitioner_id and row[5].value == week and row[2].value == appt_type:
         #       row_to_update = row
        #        break

        # If the row is found, update the values
       # if row_to_update:
          #  if new_target_income is not None:
         #       row_to_update[6].value = new_target_income  # Update Target Income
        #    if new_target_hours is not None:
       #         row_to_update[7].value = new_target_hours  # Update Target Hours

            # Save the updated file
      #      workbook.save(target_file_path)
     #       st.success(f"Successfully updated Practitioner {practitioner_id}'s target for Week {week} and Appt. Type {appt_type}.")
    #    else:
   #         st.warning(f"No matching record found for Practitioner {practitioner_id} in Week {week} with Appt. Type {appt_type}.")
  #  except Exception as e:
...
#def master_update_target():
    ...
    # Input fields for updating target data
    #practitioner_id = st.text_input("Practitioner ID", key="practitioner_id_input")
   # week = st.number_input("Week", min_value=1, max_value=52, key="week_input")
  #  appt_type = st.text_input("Appointment Type", key="appt_type_input")
...
#    new_target_hours = st.number_input("New Target Hours", min_value=0, step=1, key="new_target_hours_input")
   ...
   # if st.button("Update Target", key="update_target_button"):
    #    if practitioner_id and appt_type:
  #          update_target(practitioner_id, week, appt_type, new_target_income, new_target_hours)
 #       else:
#            st.error("Please enter all required fields (Practitioner ID, Week, Appointment Type).")
...
# If the user is a 'secondary' user, show personalized dashboard
   ...
   # Chart 1: Weekly Total Hours Worked vs. Target Hours by Appointment Type
                   ...
                   #     labels={'value': 'Hours', 'variable': 'Legend', 'Week': 'Week', 'Appt. Type': 'Appointment Type'}
                   #)
                   # st.plotly_chart(hours_worked_chart)
